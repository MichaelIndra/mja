...
import { QueryOvertimeEmployee } from '../report/query.dto';
...
export class PaySlipController implements CrudController<PaySlip> {
  ...
  @Get('/custom/getPayslipOvertime')
  async getPayslipOvertime(@Query() query: QueryOvertimeEmployee) {
    try {
      const res = await this.service.getReportOvertimeEmployee(query);
      const totalOvertime = await this.service.getTotalOvertimeCustom(query);
      return {
        ...res,
        report: totalOvertime,
      };
