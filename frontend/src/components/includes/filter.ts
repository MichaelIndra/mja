...
export default class Filters extends Vue {
  ...
  date2: boolean = false;
  monthStart: string = '';
  firstOfMonth: string | any = formatDate(
    moment()
      .startOf('month')
      .format('YYYY-MM-DD'),
    'long',
  );
  endOfMonth: string | any = formatDate(
    moment()
      .endOf('month')
      .format('YYYY-MM-DD'),
    'long',
  ...
  searchMonthStart() {
    if (this.monthStart === '') {
      ...
      this.setQueryMonth(this.monthStart);
      // this.checkFilter.dateStart = true;
    }
  }
  ...
  setInitFieldFilter() {
    switch (this.options.type) {
      ...
      case 'report_payslip_deductions':
        this.filterDepartment = 'employee.department_id';
        this.filterGroup = 'employee.group_id';
        this.filterArea = 'employee.area_id';
        this.filterPosition = 'employee.position_id';
  ...
  setEmitData(params: boolean | any) {
    ...
    switch (this.options.type) {
      ...
      case 'report_payslip_deductions':
        if (params) {
          ...
          this.$emit('getListData', InitQueryDeductions);
        }
        break;
      case 'report_payslip_overtime':
        if (params) {
      ...
      case 'report_payslip_outcome':
        if (params) {
          ...
          this.$emit('getListData', InitQueryOutcome);
        }
        break;
    }
  }
  ...
  setQueryMonth(month: string) {
    const dateStart = moment(month)
      .startOf('month')
      .subtract(7, 'days')
      .locale('id')
      .format('YYYY-MM-DD 00:00:00');
    const dateEnd = moment(month)
      .endOf('month')
      .add(7, 'days')
      .locale('id')
      .format('YYYY-MM-DD 23:59:59');
    const firstOfMonth = moment(month)
      .startOf('month')
      .locale('id')
      .format('YYYY-MM-DD 00:00:00');
    const lastOfMonth = moment(month)
      .endOf('month')
      .locale('id')
      .format('YYYY-MM-DD 23:59:59');
    ...
    this.firstOfMonth = formatDate(firstOfMonth, 'long');
    this.endOfMonth = formatDate(lastOfMonth, 'long');
    this.setEmitData(true);
  }
